...
│                                                                                        │
│   ┌─────────────────┐           ┌──────────────────┐           ┌───────────────────┐   │
│   │  PySide6 Desktop│──────────▶│  FastAPI Backend │◀──────────│  Next.js Web App  │   │
│   │   Application   │   REST    │   (uvicorn)      │   REST    │   (App Router)    │   │
│   │                 │   + JWT   │                  │   + JWT   │                   │   │
│   │  • Camera feed  │           │  /auth/*         │           │  • Dashboard      │   │
│   │  • Image input  │           │  /inspections/*  │           │  • History & View │   │
│   │  • Batch folder │           │  /reviews/*      │           │  • Review Queue   │   │
│   │  • Model compare│           │  /models/*       │           │  • Analytics      │   │
│   │  • Offline store│           │  /stats          │           │  • Compliance Rep.│   │
│   └────────┬────────┘           │  /reports/*      │           │  • Audit Trail    │   │
│            │                    │  /notifications  │           │  • RBAC & Users   │   │
│   ┌────────▼─────────┐          │  /audit-logs     │           └───────────────────┘   │
│   │   SQLite (local) │          │  /users          │                                   │
│   │   Offline-first  │          └────────┬─────────┘           ┌───────────────────┐   │
│   │   inspection log │                   │                     │    Flutter App    │   │
│   └──────────────────┘          ┌────────┴──────────┐          │     (Mobile)      │   │
│                                 │ Supabase Postgres │◀─────────┤                   │   │
│                                 │ + Supabase Storage│          │  • Mobile Monitor │   │
│                                 │ (images/metadata) │          └───────────────────┘   │
│                                 └───────────────────┘                                  │
│                                                                                        │
│   ┌────────────────────────────────────────────────────────────────────────────────┐   │
...
│   │   BaseDetector ──── Tiling · Class-aware NMS · Severity Grading                │   │
│   └────────────────────────────────────────────────────────────────────────────────┘   │
└────────────────────────────────────────────────────────────────────────────────────────┘
```
...
### 4. Launch the Web Platform (Next.js)

The web platform provides a centralized quality control, analytics, and inspection management interface.
...
Open [http://localhost:3000](http://localhost:3000) to access the dashboard. Log in with the administrator credentials seeded from `.env` (`admin@example.com` / `changeme`).

### 5. Run Mobile Companion App (Flutter)
